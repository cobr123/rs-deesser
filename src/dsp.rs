...
impl Fir {
    ...
    pub fn simd_mult_and_sum(a: &[f32], b: &[f32]) -> f32 {
        ...
        // Chunk your slices into 16-element segments using stable 'as_chunks'
        let (a_chunks, a_remainder) = a.as_chunks::<16>();
        let (b_chunks, b_remainder) = b.as_chunks::<16>();

        let mut sum_vector = f32x16::ZERO;

        // 1. SIMD Loop: Multiplies 16 lanes at once and accumulates vertically
        for (a_chunk, b_chunk) in a_chunks.iter().zip(b_chunks.iter()) {
            let va = f32x16::from(*a_chunk);
            let vb = f32x16::from(*b_chunk);
            sum_vector += va * vb;
        }
        ...
        // 3. Clean up leftover elements (if array size is not a multiple of 16)
